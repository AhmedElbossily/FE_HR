...
//------------------------------------------------------------------------ Ahmed's Testign Methods
particle_gpu *setup_ring_contact_two_deformable_parts(int nbox, grid_base **grid)
{
	printf("ATTENTION \n");
	printf("Did you Deactivate the Merge Method? \n");
...
/*  	{
	...
	//----------------------------------------------------------------------
	vec3_t bbmin_tool(0        , -1.5*ro, -1.5*ro);
	vec3_t bbmax_tool(0.25 * ro,  1.5*ro,  1.5*ro);
	auto cpu_tool = new tool_3d();
	std::vector<vec3_t> samples;
	samples = cpu_tool->sample(dx, bbmin_tool, bbmax_tool);
	...
	float4_t *pos_f = new float4_t[n + n_tool];
	...
	for (auto &it : samples)
	{
		...
		cur_pos.z = it.z;
		cur_pos.w = 1.0;
		pos_f[part_iter] = cur_pos;

		rho[part_iter] = phys.rho0;
		h[part_iter] = hdx * dx;
		vel[part_iter].x = 0.;
		vel[part_iter].y = 0.;
		vel[part_iter].z = 0.;
		T[part_iter] = joco.Tref;
		tool_p[part_iter] = 1;
		cp[part_iter] = trml.cp;
		k[part_iter] = trml.k;

		fixed[part_iter] =0.0;
		

		/*  if (cur_pos.x > 0.9 * (bbmax_tool.x - bbmin_tool.x))
			fixed[part_iter] = 10.;
		else
			fixed[part_iter] = 0.0; */

		float_t l = 2.0 *1.5 * ro;

		if (cur_pos.z < -l / 2 + dx / 2)
		{
			fixed[part_iter] = 2;
		}

		if (cur_pos.z > +l / 2 - 2.*dx)
		{
			fixed[part_iter] = 2;
		}

		if (cur_pos.y < - l / 2 + dx / 2)
		{
			fixed[part_iter] = 2;
		}

		if (cur_pos.y > +l / 2 - 2*dx )
		{
			fixed[part_iter] = 2;
		} 
	...
	// particle_gpu *particles = new particle_gpu(pos_f, vel, rho, T, h, fixed, tool_p, n+n_tool);
	particle_gpu *particles = new particle_gpu(pos_f, vel, rho, T, h, fixed, tool_p, n + n_tool, cp, H, k);

	*grid = new grid_gpu_green(n + n_tool, make_float3_t(-.2, -2.*ro, -2.*ro), make_float3_t(+.1, 2.*ro, 2.*ro), hdx * dx);
	global_blanking = new blanking(		    vec3_t      (-.2, -2.*ro, -2.*ro), vec3_t       (+.1, 2.*ro, 2.*ro));
	...
	global_tool[0]->set_mu(0.35);
	global_tool[0]->set_rod(rod);

	actions_setup_corrector_constants(corr);
	actions_setup_physical_constants(phys);
	actions_setup_johnson_cook_constants(joco,joco);
	actions_setup_thermal_constants_substrate(trml);
	actions_setup_thermal_constants_rod(trml);
	actions_setup_rod_constants(rod);

	interactions_setup_corrector_constants(corr);
	interactions_setup_physical_constants(phys);
	interactions_setup_johnson_cook_constants(joco,joco);
	interactions_setup_thermal_constants_substrate(trml);
	interactions_setup_thermal_constants_rod(trml, global_tool[0]);
	interactions_setup_geometry_constants(*grid);
...
particle_gpu *setup_friction_cub(int nbox, grid_base **grid)
{
	printf("ATTENTION \n");
	printf("Did you Deactivate the Merge Method? \n");
	printf("Did you change the boundary condition method?\n");
	...
	float_t sub_thickness = 0.25 * ro;
	float_t sub_width     =  3.  *ro;
	float_t sub_length    =  5.  *ro;

	float_t dx = 2 * ro / (nbox - 1);
	float_t hdx = 1.7;

	float_t spacing = ro + 1.01*dx;
	...
	float_t vel_ring = ro * rod.Vsf ;
	float_t vx       = .01 *ro * rod.Vsf ;
...
/*  	{
	...
	for (int i = 0; i < nbox; i++)
	{
		for (int j = 0; j < nbox; j++)
		{
			for (int k = 0; k < nbox; k++)
			{
				...
				float_t pz = -ro + k * dx;
				float_t dist = sqrt(px * px + py * py + pz * pz);
				if ( abs(px)>=ri || abs(py)>=ri ||abs(pz)>=ri)
				{
					float4_t posl, posr;
	...
	float_t delta_t_max = 0.8 * hdx * dx / (vel_ring + c0);
	global_time_dt = 0.5 * delta_t_max;
	global_time_final =  sub_length / vel_ring;

	printf("calculating with %d\n", n);

	//----------------------------------------------------------------------
	vec3_t bbmin_tool(0        	   , -sub_width/2., - ro-10*dx);
	vec3_t bbmax_tool(sub_thickness,  sub_width/2.,  sub_length);
	auto cpu_tool = new tool_3d();
	std::vector<vec3_t> samples;
	samples = cpu_tool->sample(dx, bbmin_tool, bbmax_tool);
	int n_tool = samples.size();
...
/* 		if (cur_pos.x > 0.8 * (bbmax_tool.x - bbmin_tool.x))
			fixed[part_iter] = 10.;
		else
			fixed[part_iter] = 0.0;  */
...
/* 		float_t l = 2.0 *1.5 * ro;
	...
	*grid = new grid_gpu_green(n + n_tool,
							   make_float3_t(-2*spacing      - 5*dx,   bbmin_tool.y - 5*dx, bbmin_tool.z - 5*dx), 
							   make_float3_t( bbmax_tool.x + 5*dx,   bbmax_tool.y + 5*dx, bbmax_tool.z + 5*dx), hdx * dx);
	global_blanking = new blanking(		    
							   vec3_t       (-2*spacing      - 5*dx,   bbmin_tool.y - 5*dx, bbmin_tool.z - 5*dx), 
							   vec3_t       ( bbmax_tool.x + 5*dx,   bbmax_tool.y + 5*dx, bbmax_tool.z + 5*dx));
