...
// power law flow stress model
__device__ double sigma_yield(double epsilon_dot, double T)
{
	// constants
	double A_solid = 7.81e19;
	double A_semiSolid = 4.8e16;
	double A = 0.5 * (A_solid + A_semiSolid);
	double Q = 0.5 * 1000. * (270.02 + 246.16);
	double R = 8.314;

	// convert T into K
	T = T + 273;
	double Z = epsilon_dot * exp(Q / (R * T));
	double Z_over_A = Z / A;
	double exponent = 1.0 / 5.67;

	double term1 = pow(Z_over_A, exponent);
	double term2 = sqrt(pow(Z_over_A, 2 * exponent) + 1);
	double term3 = term1 + term2;

	// the following sigma is in MPa
	double sigma = (1.0 / 0.011) * log(term3);

	// multiply the above by 10e6 to convert it to Pa
	return sigma * 10.e6;
}
						  ...
						  double norm_Strial, double eps_pl_init, double t, double delta_t, double mu)
{
	...
	//double sigmaY = sigma_yield(eps_pl_equiv_dot, t);
...
__device__ double solve_secant(joco_constants jc, double init, double tol,
							   double norm_Strial, double eps_pl_init, double t,
							   double delta_t, double mu, bool &failed)
{
