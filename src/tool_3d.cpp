	...
	for (unsigned int i = 0; i < nx; i++) {
		for (unsigned int j = 0; j < ny; j++) {
			for (unsigned int k = 0; k < nz; k++) {
				...
				vec3_t pos(px, py, pz);
				if (/* contact_safe(pos) */true) {  // TODO:: need to be checked
					fprintf(fp, "%f %f %f\n", px, py, pz);
...
const std::vector<vec3_t> tool_3d::sample(float_t dx,vec3_t &bbmin, vec3_t &bbmax) const {
...
/* 	vec3_t bbmin, bbmax;
	...
	unsigned int nx = ceil(lx/dx)+1;
	unsigned int ny = ceil(ly/dx)+1;
	unsigned int nz = ceil(lz/dx)+1;
