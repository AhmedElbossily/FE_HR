...
__global__ void do_calculate_center_of_mass(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
											const float_t *__restrict__ rho, float_t *__restrict__ p, float3_t *__restrict__ cfm,
											float_t *__restrict__ h, int *__restrict__ surface, int N, float_t *fixed, int step)
{
	...
	float3_t cfmi = cfm[pidx];
	float_t hi = h[pidx];
	int surfacei = 0.0;

	float_t magni = sqrtf(cfmi.x * cfmi.x + cfmi.y * cfmi.y + cfmi.z * cfmi.z);

	if (magni > 0.005 * hi && magni != 0.0)
	{
		surfacei = 1;
		cfmi.x = cfmi.x / magni;
		cfmi.y = cfmi.y / magni;
	...
	surface[pidx] = surfacei;
	cfm[pidx] = cfmi; // now cfm became ni
}

__global__ void do_update_cp_and_k(particle_gpu particles)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;
	...
	if (T > johnson_cook_rod.Tmelt)
		return;

	if (part == 0)
	{
		// https://www.researchgate.net/publication/350139376_Part_deflection_and_residual_stresses_in_laser_powder_bed_fusion_of_H13_tool_steel
		//particles.cp[pidx] = (438.23895 + 0.169 * T) * 1.0e6 * (1. / rod.ms);
		//particles.k[pidx] = (28.99665 + 0.01845 * T) * 1.0e6 * (rod.Vsf);
		
		particles.cp[pidx] = (460) * 1.0e6 * (1. / rod.ms);
		particles.k[pidx] = (24.3) * 1.0e6 * (rod.Vsf);
	...
	if (part == 2)
	{
		//particles.cp[pidx] = (579.301 + (470.8682 - 579.301) / (1 + pow(T / 395.2813, 4.144536))) * 1.0e6 * (1. / rod.ms);
		//particles.k[pidx] = (30.53824 + (42.61018 - 30.53824) / (1 + pow(T / 436.7859, 4.28974))) * 1.0e6 * (rod.Vsf);		
		//particles.k[pidx] = 30.53824 * 1.0e6 * (rod.Vsf);
               
                particles.cp[pidx] = 519. * 1.0e6 * (1. / rod.ms);
                particles.k[pidx] = 42.6 * 1.0e6 * (rod.Vsf);
	...
	float_t Ei = Ei_init - k_Ei * (T - johnson_cook_rod.Tref);
	if(Ei < 8.e9) Ei = 8.e9;*/
	particles.E[pidx] = physics.E; 

	//particles.cp[pidx] = (812.66561 + 0.76619 * T) * 1.0e6 * (1. / rod.ms);
	//particles.k[pidx] = (168.54621 - 0.1069 * T) * 1.0e6 * (rod.Vsf);
      
      //particles.cp[pidx] = (848.73 +0.49 * T) * 1.0e6 * (1. / rod.ms);
      //if(T>500) particles.cp[pidx] = (1100) * 1.0e6 * (1. / rod.ms);
      
      //particles.k[pidx] = (93.88918 + 1.670642 * T - 0.009616162 * T * T + 0.00002321284 * T * T * T- (2.004618e-8) * T * T * T * T) * 1.0e6 * (rod.Vsf);
      //if(T>400) particles.k[pidx] = (196.) * 1.0e6 * (rod.Vsf);
      
      particles.cp[pidx] = (960.) * 1.0e6 * (1. / rod.ms);
      particles.k[pidx]=  (130.) * 1.0e6 * (rod.Vsf);
}

__global__ void do_material_eos(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
								const float_t *__restrict__ rho, float_t *__restrict__ p, int N, float_t *fixed, float_t *E)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
	...
	float_t c0 = sqrtf(physics_K / rho0);
...
__global__ void do_move_tool_particles(particle_gpu particles, vec3_t vel, float_t dt)
{
	...
	float_t vx =  6.0e-3;
	float_t vdt =   -1.* vx * dt;
	particles.pos[pidx].x = pos_x + vdt; */
...
__global__ void do_corrector_artificial_stress(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
											   ...
											   mat3x3_t *__restrict__ R, int N, float_t *fixed)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
...
/* To calculate the strain rate magnitude from the strain rate tensor,
you can use the Frobenius norm of the tensor.
The Frobenius norm of a matrix is the square root of the sum of the absolute squares of its elements.
For a 3x3 strain rate tensor  */
__device__ float_t calculateStrainRateMagnitude(const mat3x3_t &epsdot)
{
	float_t sum_of_squares = 0.0;

	// Iterate over each element of the 3x3 tensor
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			sum_of_squares += epsdot[i][j] * epsdot[i][j];
		}
	}
...
__global__ void do_material_stress_rate_jaumann(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
												...
												mat3x3_t *S_t, int N, float_t *fixed, float4_t *vel, float_t *p, float_t dt, float_t *E, float4_t *vel_bc)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
	...
	mat3x3_t epsdot = mat3x3_t(vx_x, 0.5 * (vx_y + vy_x), 0.5 * (vx_z + vz_x),
							   0.5 * (vx_y + vy_x), vy_y, 0.5 * (vy_z + vz_y),
							   0.5 * (vx_z + vz_x), 0.5 * (vy_z + vz_y), vz_z);

	vel_bc[pidx].w = calculateStrainRateMagnitude(epsdot);

	// epsdot = epsdot * float_t(1. / rod.Vsf); /////////////////////////////////////////////////////////

	mat3x3_t omega = mat3x3_t(0., 0.5 * (vy_x - vx_y), 0.5 * (vz_x - vx_z),
							  0.5 * (vx_y - vy_x), 0., 0.5 * (vz_y - vy_z),
							  0.5 * (vx_z - vz_x), 0.5 * (vy_z - vz_y), 0.);

	// omega = omega * float_t(1. / rod.Vsf); ///////////////////////////////////////////////////////////

	const mat3x3_t S = mat3x3_t(Sxx, Sxy, Sxz,
								Sxy, Syy, Syz,
								Sxz, Syz, Szz);

	// float_t hyd_p = p[pidx];
	vel[pidx].w += ((Sxx)*epsdot[0][0] * dt) + (Sxy * epsdot[0][1] * dt) + (Sxz * epsdot[0][2] * dt) +
				   (Sxy * epsdot[0][1] * dt) + ((Syy)*epsdot[1][1] * dt) + (Syz * epsdot[1][2] * dt) +
				   (Sxz * epsdot[0][2] * dt) + (Syz * epsdot[1][2] * dt) + ((Szz)*epsdot[2][2] * dt);
...
__global__ void do_material_fric_heat_gen(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool, const float4_t *__restrict__ vel,
										  const float3_t *__restrict__ f_fric, const float3_t *__restrict__ n, float_t *__restrict__ T, float3_t vel_tool,
										  float_t dt, int N, float_t *fixed)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
	...
	const float_t eta = trml_sub.eta;
	...
	T[pidx] += eta * dt * f_fric_mag * v_rel_mag / (trml_sub.cp * physics.mass);
}
...
__global__ void do_contmech_continuity(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
									   ...
									   float_t *__restrict__ rho_t, int N, float_t *fixed)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
	...
	// rho_t[pidx] = -rhoi * (vx_x + vy_y + vz_z) * (1. / rod.Vsf);
...
__global__ void do_contmech_momentum(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
									 ...
									 float3_t *__restrict__ vel_t, int N, float_t *fixed)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
	...
	/* veli_t.x += (Sxx_x * Sxx_x + Syx_x * Syx_x + Szx_x * Szx_x) + fcx / mass + ftx / mass;
	veli_t.y += (Sxy_y * Sxy_y + Syy_y * Syy_y + Szy_y * Szy_y) + fcy / mass + fty / mass;
	veli_t.z += (Sxz_z * Sxz_z + Syz_z * Syz_z + Szz_z * Szz_z) + fcz / mass + ftz / mass; */

	/* 	float_t ax = (fcx / mass) + (ftx / mass) + Sxx_x;
		veli_t.x += sqrt(Sxy_y * Sxy_y + Sxz_z * Sxz_z + ax * ax);

		float_t ay = (fcy / mass) + (fty / mass) + Syy_y;
		veli_t.y += sqrt(Syx_x * Syx_x + Syz_z * Syz_z + ay * ay);

		float az = (fcz / mass) + (ftz / mass) + Szz_z;
		veli_t.z += sqrt(Szx_x * Szx_x + Szy_y * Szy_y + az * az);
	 */
	/* veli_t.x += Sxx_x + fcx / mass + ftx / mass;
	veli_t.y += Syy_y + fcy / mass + fty / mass;
	veli_t.z += Szz_z + fcz / mass + ftz / mass; */
...
__global__ void do_contmech_advection(const float_t *__restrict__ blanked, const float_t *__restrict__ in_tool,
									  ...
									  float3_t *__restrict__ pos_t, int N, float_t *fixed)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= N)
		return;
...
__global__ void do_check_valid_full(particle_gpu particles)
{
	...
	invalid = invalid || isnaninf(particles.ft[pidx].x);
	invalid = invalid || isnaninf(particles.ft[pidx].y);
	invalid = invalid || isnaninf(particles.ft[pidx].z);

	invalid = invalid || isnaninf(particles.fc[pidx].x);
	invalid = invalid || isnaninf(particles.fc[pidx].y);
	invalid = invalid || isnaninf(particles.fc[pidx].z);

	if (invalid)
	{
		printf("found particle with nan values that is not blanked!\n");
		particles.blanked[pidx] = 1.;
		printf("do_check_valid_full: invalidated particle %d due to nan!\n", pidx);
	}
}
...
__global__ void do_plasticity_johnson_cook(particle_gpu particles, float_t dt)
{
	...
	bool is_billet = (part == 1) ? true : false;

	float_t physics_G = particles.E[pidx] / (2. * (1. + physics.nu));
	float_t mu = physics_G;
	...
	if (is_billet)
	{
		cT = johnson_cook_sub.clamp_temp;
		Tm = johnson_cook_sub.Tmelt;
	}
	else
	{
		cT = johnson_cook_rod.clamp_temp;
		Tm = johnson_cook_rod.Tmelt;
	}
	...
	sigma_Y = sigma_yield(johnson_cook_sub, eps_pl, eps_pl_dot, T);
	/* if (eps_pl_dot <= 0.)
	{
		sigma_Y = sigma_yield(10.e-13, T);
	}
	else
	{
		sigma_Y = sigma_yield(eps_pl_dot, T);
	...
	bool failed = false;
	float_t delta_lambda;

	if (is_billet)
	{
		delta_lambda = solve_secant(johnson_cook_sub, fmax(eps_pl_dot * dt * sqrt(2. / 3.), 1e-8), 1e-6, norm_Strial, eps_pl, T, dt, physics_G, failed);
	}
	else
	{
		delta_lambda = solve_secant(johnson_cook_rod, fmax(eps_pl_dot * dt * sqrt(2. / 3.), 1e-8), 1e-6, norm_Strial, eps_pl, T, dt, physics_G, failed);
	}
	...
	float_t eps_pl_dot_new = (sqrtf(2.0 / 3.0) * fmaxf(delta_lambda, 0.) / dt) / rod.Vsf;
	...
	if (trml_tq != 0.)
	{
		...
		sigma_Y = sigma_yield(johnson_cook_sub, eps_pl_new, eps_pl_dot_new, T);

		float_t rho = particles.rho[pidx];
		float_t delta_T = trml_tq / (cp * rho) * delta_eps_pl * sigma_Y;
		particles.T[pidx] += (delta_T);
		// particles.T[pidx] += (delta_T/15.);
	}
}

__global__ void do_boundary_conditions_thermal(particle_gpu particles, float_t dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;

	if (particles.fixed[pidx] == 7)
	{
		float_t T = particles.T[pidx];
		float_t T_ref = trml_sub.T_init;
		float_t h_conv;

		if (T < T_ref)
			h_conv = 0.;
		else
			h_conv = 25.* rod.Vsf  * 1.e3; // 25

		float_t As = rod.dz * rod.dz;
		float_t q_dot = h_conv * (T_ref - T) * As;

		float_t cp = particles.cp[pidx];
		float_t mass = particles.rho[pidx] * rod.dz * rod.dz * rod.dz;
		float_t is_tool_particle_j = particles.tool_particle[pidx];
		particles.T[pidx] += (q_dot * dt) / (cp * mass);
	}
}

__global__ void do_merge_conditions_thermal(particle_gpu particles, int *joined_count)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;
	if (particles.tool_particle[pidx] == 1.0)
		return;

	if ((particles.T[pidx] > 839.5) && (particles.eps_pl[pidx] > 0.01))
	{
		particles.tool_particle[pidx] = 1.0; // substrate
		particles.pos[pidx].w = 2.0;
	}
}
...
__global__ void do_boundary_conditions(particle_gpu particles, float_t dt, int *joined_count, float_t *joined_mean, float_t v_die, float_t gWz)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;
	...
	if (particles.tool_particle[pidx] == 2 || particles.fixed[pidx] == 8)
	{

		particles.vel[pidx].x = 0.;
		particles.vel[pidx].y = 0.0;
		particles.vel[pidx].z = 0.0;

		particles.fc[pidx].x = 0.0;
		particles.fc[pidx].y = 0.0;
		particles.fc[pidx].z = 0.0;

		particles.pos_t[pidx].x = 0.0;
		particles.pos_t[pidx].y = 0.0;
		particles.pos_t[pidx].z = 0.0;

		particles.vel_t[pidx].x = 0.0;
		particles.vel_t[pidx].y = 0.0;
		particles.vel_t[pidx].z = 0.0;
	}
	...
	if (particles.tool_particle[pidx] == 0)
	{
		...
		particles.vel[pidx].z = v_die;
		particles.pos[pidx].z += v_die * dt;
	}
}

__global__ void do_rpm_positon_update(particle_gpu particles, float_t dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;
	...
	if (particles.tool_particle[pidx] == 0)
	{
		...
		glm::vec3 w(0.0, 0.0, rod.wz);
		glm::vec3 r(particles.pos[pidx].x, particles.pos[pidx].y, 0.0);
		glm::vec3 v = glm::cross(w, r);

		particles.vel[pidx].x = v.x;
		particles.vel[pidx].y = v.y;
		particles.vel[pidx].z = rod.vz;

		particles.pos[pidx].x += v.x * dt;
		particles.pos[pidx].y += v.y * dt;
		particles.pos[pidx].z += rod.vz * dt;
...
__global__ void do_boundary_conditions_small_problem(particle_gpu particles, float_t dt, int *joined_count, float_t *joined_mean)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N)
		return;
...
__global__ void do_blanking(particle_gpu particles, float_t vel_max_squared, float3_t bbmin, float3_t bbmax)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
...
void material_eos(particle_gpu *particles)
{
	...
	dim3 dB(block_size);
	do_material_eos<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->rho, particles->p, particles->N, particles->fixed, particles->E);
...
void corrector_artificial_stress(particle_gpu *particles)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_corrector_artificial_stress<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->rho,
											   particles->p, particles->S, particles->R, particles->N,
											   particles->fixed);
...
void material_stress_rate_jaumann(particle_gpu *particles)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_material_stress_rate_jaumann<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->v_der,
												particles->S, particles->S_t, particles->N, particles->fixed, particles->vel, particles->p,
												global_time_dt, particles->E, particles->vel_bc);
...
void contmech_continuity(particle_gpu *particles)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_contmech_continuity<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->rho, particles->v_der,
									   particles->rho_t, particles->N, particles->fixed);
...
void contmech_momentum(particle_gpu *particles)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_contmech_momentum<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->S_der,
									 particles->fc, particles->ft, particles->vel_t, particles->N, particles->fixed);
...
void perform_boundary_conditions_thermal(particle_gpu *particles)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_boundary_conditions_thermal<<<dG, dB>>>(*particles, global_time_dt);
	check_cuda_error("After boundary_conditions_thermal\n");
}

void perform_merge_conditions_thermal(particle_gpu *particles, int *joined_count)
{
	if (!m_thermal)
		return;
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_merge_conditions_thermal<<<dG, dB>>>(*particles, joined_count);
	check_cuda_error("After merge_conditions_thermal\n");
}

void perform_boundary_conditions(particle_gpu *particles, int *joined_count, float_t *joined_mean)
{
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);

	do_boundary_conditions<<<dG, dB>>>(*particles, global_time_dt, joined_count, joined_mean, global_die_velocity, global_wz);
	check_cuda_error("After boundary_conditions\n");

	int sub_Steps = 1000;
	float_t dt = global_time_dt / sub_Steps;

	/* for(int i = 0; i <sub_Steps; i++)
	{
		do_rpm_positon_update<<<dG,dB>>>(*particles, dt);
		check_cuda_error("After do_rpm_positon_update\n");
...
void perform_blanking(particle_gpu *particles, blanking *global_blanking)
{
	...
	do_blanking<<<dG, dB>>>(*particles, global_blanking->get_max_vel_squared(), make_float3_t(bbmin.x, bbmin.y, bbmin.z), make_float3_t(bbmax.x, bbmax.y, bbmax.z));
...
void actions_setup_rod_constants(rod_constants rod_h)
{

	cudaMemcpyToSymbol(rod, &rod_h, sizeof(rod_h), 0, cudaMemcpyHostToDevice);
}
...
void actions_setup_johnson_cook_constants(joco_constants johnson_cook_sub_h, joco_constants johnson_cook_rod_h)
{
...
void actions_setup_johnson_cook_constants(joco_constants johnson_cook_sub_h)
{
	cudaMemcpyToSymbol(johnson_cook_sub, &johnson_cook_sub_h, sizeof(joco_constants), 0, cudaMemcpyHostToDevice);
	m_plastic = true;
}

void actions_setup_thermal_constants_substrate(trml_constants thermal_h)
{
	cudaMemcpyToSymbol(trml_sub, &thermal_h, sizeof(trml_constants), 0, cudaMemcpyHostToDevice);
	if (thermal_h.tq != 0.)
	{
	...
	if (thermal_h.alpha != 0.)
	{
		...
		printf("Action wp warning! heat conduction constants set but no heat conduction algorithm active!");
#endif
	}
}

void actions_setup_thermal_constants_rod(trml_constants thermal_h)
{
	cudaMemcpyToSymbol(trml_rod, &thermal_h, sizeof(trml_constants), 0, cudaMemcpyHostToDevice);
...
#if !(defined(Thermal_Conduction_Brookshaw) || defined(Thermal_Conduction_PSE))
		printf("Action tool warning! heat conduction constants set but no heat conduction algorithm active!");
#endif
	}
}
...
void material_fric_heat_gen(particle_gpu *particles, vec3_t vel)
{
	...
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_material_fric_heat_gen<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->vel, particles->ft, particles->cfm,
...
void calculate_center_of_mass(particle_gpu *particles, int step)
{
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_calculate_center_of_mass<<<dG, dB>>>(particles->blanked, particles->tool_particle, particles->rho, particles->p, particles->cfm,
											particles->h, particles->surface, particles->N, particles->fixed, step);
	check_cuda_error("After calculate_center_of_mass\n");
}

void update_cp_and_k(particle_gpu *particles)
{
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size - 1) / block_size);
	dim3 dB(block_size);
	do_update_cp_and_k<<<dG, dB>>>(*particles);
	check_cuda_error("After do_update_cp\n");
}
