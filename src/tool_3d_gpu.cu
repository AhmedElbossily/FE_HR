...
__global__ void do_compute_contact_forces_fraser( particle_gpu particles, float_t dt, int* joined_count, float_t* joined_mean) {	
	...
	if (pidx >= N) return;
	if (particles.blanked[pidx] == 1.) return;
	if (particles.surface[pidx] == 0 ) return;
	if (particles.tool_particle[pidx] == 1.) return; // will be removed when the contact force applied to it

	float4_t pos = particles.pos[pidx];
	float_t penetration = particles.pN[pidx];
	...
	qp.z = pos.z;
	vec3_t fN(0.,0.,0.);
	vec3_t nis(particles.cfm[pidx].x,particles.cfm[pidx].y, particles.cfm[pidx].z);
	 
/* 	float_t surface_level;
	if(joined_count[0] !=0){ 
		surface_level =  float_t(joined_mean[0]/joined_count[0]) - 1.0* rod.dz; 
	}
	else surface_level =  - 1.0* rod.dz;  */
	//printf(" float_t(-joined_mean[0]/joined_count[0]): %lf\n",  float_t(-joined_mean[0]/joined_count[0]) );
	//float_t surface_level = float_t(-joined_count[0]/rod.move_sub)* 1.0* rod.dz - 1.0* rod.dz;
	//float_t surface_level = particles.vel[pidx].w - 0.1*rod.dz;
	float_t surface_level =  - 1.0* rod.dz;

	if(qp.z > surface_level){
		float_t pd = qp.z - surface_level;
		float_t PFAC = 0.1;
		float_t kij = ((slave_mass*slave_mass)/(slave_mass+slave_mass))*(PFAC/(dt*dt));

		// calculating delta penetration
		float_t dpN = pd - penetration;
		float_t DFAC = 0.1;
		float_t cij = DFAC *(slave_mass+slave_mass) *sqrt((kij*(slave_mass+slave_mass))/(slave_mass*slave_mass));

		particles.pN[pidx]=pd;

		fN.x = - (kij*pd + cij*dpN)*nis.x;
		fN.y = - (kij*pd + cij*dpN)*nis.y;
		fN.z = - (kij*pd + cij*dpN)*nis.z;
		
	}else{
		particles.pN[pidx]=0.0;
	}
...
__global__ void do_free_fall( particle_gpu particles) {	
	...
	if (pidx >= N) return;
	if (particles.blanked[pidx] == 1.) return;
	if (particles.tool_particle[pidx] == 0.0) return; 

	float4_t pos = particles.pos[pidx];
	float_t temp = particles.T[pidx];
	float_t h = particles.h[pidx];
	vec3_t fN(0.,0.,0.);
	...
	//if(qp.z < -h && (temp >= 673 || particles.eps_pl[pidx] >= 0.01)){
	if(qp.z < -h && (temp >= 673.0)){

		fN.x = 0.;
		fN.y = 0.;
		fN.z = slave_mass * 9.81;
	}
...
void tool_3d_gpu::set_rod(rod_constants rod_h) {
...
/* tool_3d_gpu::tool_3d_gpu(vec3_t vel, phys_constants phys, float_t alpha) {
	if (phys.mass <= 0.) {
		printf("alarm!, tool was given a particle mass of zero. no contact forces will be computed!\n");
	}
	cudaMemcpyToSymbol(contact_alpha, &alpha,     sizeof(float_t), 0);
...
void tool_3d_gpu::compute_contact_force_fraser(particle_gpu *particles, int* joined_count, float_t* joined_mean){
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size-1) / block_size);
	dim3 dB(block_size);

 	do_compute_contact_forces_fraser<<<dG,dB>>>( *particles, global_time_dt, joined_count, joined_mean); 
	check_cuda_error("compute contact force fraser!\n");
}

void tool_3d_gpu::free_fall(particle_gpu *particles){
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N + block_size-1) / block_size);
	dim3 dB(block_size);

 	do_free_fall<<<dG,dB>>>( *particles); 
	check_cuda_error("free fall!\n");
}
