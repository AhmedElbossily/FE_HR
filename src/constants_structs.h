...
struct trml_constants {
	...
	float_t cpMax;  // max thermal capacity 
	float_t cp0;  // max thermal capacity 
	float_t cp1;     // thermal capacity first const
	float_t cp2;     // thermal capacity seconf const
	float_t tq;	    // taylor quinney constant
	float_t k;	    // thermal conductivity
	float_t k0;	    // thermal conductivity
	float_t k1;	    // thermal conductivity
	float_t k2;	    // thermal conductivity
	float_t k3;	    // thermal conductivity
	float_t alpha;	// thermal diffusitivty
	float_t T_init;	// initial temperature
	float_t eta;	// fraction of frictional power turned to 
	float_t L;		// latent heat
...
//geometrical constants for rod 
struct rod_constants{
	...
	float_t ms;
	int     move_sub;
	float_t orifice_radius;

};

rod_constants make_rod_constants();
