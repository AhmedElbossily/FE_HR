...
public:
	...
	void set_rod(rod_constants rod_h);
	...
	void compute_contact_force_fraser(particle_gpu *particles, int* joined_count, float_t* joined_mean);

	void free_fall(particle_gpu *particles);
	...
	tool_3d_gpu();
	//tool_3d_gpu(vec3_t vel, phys_constants phys, float_t alpha = 0.1);
	tool_3d_gpu(vec3_t &vel, phys_constants phys, float_t alpha = 0.1);
