    ...
    int num_part)
{
    ...
    if (idx < num_part)
    {
        ...
        float_t cp = particles.cp[sorted_index];
        float_t k = particles.k[sorted_index];
        float_t E = particles.E[sorted_index];
        ...
        float3_t cfm = particles.cfm[sorted_index];
        float_t pN = particles.pN[sorted_index];
        int surface = particles.surface[sorted_index];
        int unique_idx = particles.unique_idx[sorted_index];
        ...
        buffer.cfm[idx] = cfm;
        buffer.pN[idx] = pN;
        buffer.surface[idx] = surface;
        buffer.cp[idx] = cp;
        buffer.k[idx] = k;
        buffer.E[idx] = E;
        buffer.unique_idx[idx] = unique_idx;
    }
}
...
__global__ static void copy_from_buffer(particle_gpu particles, grid_gpu_green::device_buffer buffer, int N)
{
    ...
    particles.cp[idx] = buffer.cp[idx];
    particles.k[idx] = buffer.k[idx];
    particles.E[idx] = buffer.E[idx];

    particles.cfm[idx] = buffer.cfm[idx];
    particles.pN[idx] = buffer.pN[idx];
    particles.surface[idx] = buffer.surface[idx];
    particles.unique_idx[idx] = buffer.unique_idx[idx];
}
...
static void double_buffer(particle_gpu *particles, grid_gpu_green::device_buffer *buffer)
{
    ...
    std::swap(particles->cfm, buffer->cfm);
    std::swap(particles->pN, buffer->pN);
    std::swap(particles->surface, buffer->surface);
    std::swap(particles->cp, buffer->cp);
    std::swap(particles->k, buffer->k);
    std::swap(particles->E, buffer->E);
    std::swap(particles->unique_idx, buffer->unique_idx);
}
...
void grid_gpu_green::alloc_buffer(int num_cell, int num_part)
{
    ...
    cudaMalloc((void **)&m_buffer->cp, sizeof(float_t) * num_part);
    cudaMalloc((void **)&m_buffer->k, sizeof(float_t) * num_part);
    cudaMalloc((void **)&m_buffer->E, sizeof(float_t) * num_part);

    cudaMalloc((void **)&m_cell_start, sizeof(int) * num_cell);
    cudaMalloc((void **)&m_cell_end, sizeof(int) * num_cell);

    cudaMalloc((void **)&m_buffer->cfm, sizeof(float3_t) * num_part);
    cudaMalloc((void **)&m_buffer->pN, sizeof(float_t) * num_part);
    cudaMalloc((void **)&m_buffer->surface, sizeof(int) * num_part);
    cudaMalloc((void **)&m_buffer->unique_idx, sizeof(int) * num_part);
}

grid_gpu_green::grid_gpu_green(unsigned int max_cell, unsigned int N) : grid_base(max_cell, N)
{

    alloc_buffer(max_cell, N);
}

grid_gpu_green::grid_gpu_green(int num_part, float3_t bbmin, float3_t bbmax, float_t h) : grid_base(num_part, bbmin, bbmax, h)
{
