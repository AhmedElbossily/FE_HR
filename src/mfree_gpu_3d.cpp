...
int main(int argc, char *argv[])
{
	...
	printf("Number of available GPU: %d \n", deviceCount);
	...
	/*  if (deviceCount < 2) {
				printf("Error: There are not enough GPUs available.\n");
				return 1;
			}
			cudaSetDevice(1);
			printf("The second GPU was slected \n");
		*/
	...
	ret = system("rm results/*.csv");

	ret = system("rm temp/*.txt");
	ret = system("rm temp/*.vtk");
	ret = system("rm temp/*.csv");

	ret = system("rm Results_sammary/*.txt");
	ret = system("rm Results_sammary/*.vtk");
	ret = system("rm Results_sammary/*.csv");
	...
	// 	Ahmed's Added Testing
	//	particle_gpu *particles = setup_ring_contact_two_deformable_parts(80, &grid);
	//	particle_gpu *particles = setup_friction_cub(40, &grid);
	...
	//  particle_gpu *particles = setup_single_grain_1tool_trml_steady_si_units(&grid);
	//   particle_gpu *particles = setup_single_grain_1tool_trml_steady_si_units_scaling_down(&grid);
	// particle_gpu *particles = setup_FSE(&grid);
	float_t p_mass = 0.;
	// particle_gpu *particles = setup_FSE_Cylinder(&grid, p_mass);
	//particle_gpu *particles = setup_FSE_Cylinder_gcms(&grid, p_mass);
	particle_gpu *particles = setup_FEx_g_mm_sec(&grid, p_mass);


	// particle_gpu *particles = setup_FSE_cm_g_s(&grid) ;
	...
	leap_frog *stepper = new leap_frog(particles->N, grid->num_cell(), grid->gd(), p_mass);
	...
	for (int step = 0; step < last_step; step++)
	{
		...
		// bool record_this_step =true;

		if (step > 0)
		{
			stepper->step(particles, grid, (record_this_step && global_tool_forces != 0), step);
			// if(step == 2) exit(0);
		}
		...
		if (record_this_step)
		{
			vtk_writer(particles, stepper->_p_mass); // write particles & tool(s) to VTK
			check_cuda_error();
			...
			stepper->report_foce(step);
			//stepper->report_internal_kinetic_energy(particles);
