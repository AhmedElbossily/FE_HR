...
particle_gpu::particle_gpu(unsigned int N) {
	...
	cudaMalloc((void **) &pN,   sizeof(float_t)*N);
	cudaMalloc((void **) &E,   sizeof(float_t)*N);
	...
	cudaMalloc((void**) &cp, sizeof(float_t)*N);
	...
	cudaMalloc((void **) &unique_idx, sizeof(int)*N);

	thrust::device_ptr<int> t_idx(this->idx);
	thrust::sequence(t_idx, t_idx+N);

	thrust::device_ptr<int> t_unique_idx(this->unique_idx);
	thrust::sequence(t_unique_idx, t_unique_idx+N);

	cudaMalloc((void**) &num_nbh, sizeof(int)*N);
	cudaMalloc((void**) &cfm, sizeof(float3_t)*N);
	cudaMalloc((void**) &surface, sizeof(int)*N);
	...
	cudaMemset(pN,0,   sizeof(float_t)*N);
	cudaMemset(E,0,   sizeof(float_t)*N);
	...
	cudaMemset(cfm, 0,  sizeof(float3_t)*N);
	cudaMemset(surface, 0,  sizeof(int)*N);
...
particle_gpu::particle_gpu(float4_t *pos, float4_t *vel_init, float_t *rho, float_t *T_init, float_t *h, float_t *fixed, float_t *tool_p, 
							unsigned int N, float_t*h_cp, float_t* E, float_t*k): particle_gpu(pos, vel_init, rho, T_init, h, fixed, N) {

	cudaMemcpy(this->tool_particle, tool_p, sizeof(float_t)*N, cudaMemcpyHostToDevice);
	cudaMemcpy(this->cp, h_cp, sizeof(float_t)*N, cudaMemcpyHostToDevice);
	cudaMemcpy(this->k, k, sizeof(float_t)*N, cudaMemcpyHostToDevice);
	cudaMemcpy(this->E, E, sizeof(float_t)*N, cudaMemcpyHostToDevice);
}
